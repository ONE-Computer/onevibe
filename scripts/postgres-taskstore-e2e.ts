...
const main = async () => {
  ...
  try {
    ...
    const operationAttachments = JSON.stringify([{ name: 'operation.txt', mimeType: 'text/plain', dataBase64: Buffer.from('operation bytes').toString('base64') }])
    const operation = await first.createFollowUpOperation(task.id, 'taskstore-follow-up-1', 'c'.repeat(64), 'Resume the durable operation.', operationAttachments, 'immediate', ownerUserId)
    const operationReplay = await first.createFollowUpOperation(task.id, 'taskstore-follow-up-1', 'c'.repeat(64), 'Resume the durable operation.', operationAttachments, 'immediate', ownerUserId)
    assert.equal(operation.claimed, true)
    assert.equal(operationReplay.claimed, false)
    assert.equal(operationReplay.operation.id, operation.operation.id)
    await first.updateFollowUpOperation(operation.operation, { state: 'ready', responseJson: JSON.stringify({ status: 'queued', taskId: task.id }) })
    ...
    try {
      ...
      assert.deepEqual((await second.listRecoverableFollowUpOperations()).map((candidate) => ({ id: candidate.id, state: candidate.state, taskId: candidate.taskId })), [{ id: operation.operation.id, state: 'ready', taskId: task.id }])
