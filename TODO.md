...
- [ ] **P4-02** Migrate database — Drizzle/PostgreSQL schema contract, six migrations, owner-bound conversation/task-lineage/provider-message/MCP-history/legacy-provenance tables, an isolated async Postgres chat repository, owner-required importer, organization/member staging, cross-owner relationship validation, disposable live import/restart proof, a disposable PostgreSQL migration/chat smoke proof, and a fail-closed persistence-driver guard are now present; keep open until the full TaskStore repository adapter, production migration procedure, application-level idempotency proof, and a controlled `DATABASE_URL` runtime switch are complete
