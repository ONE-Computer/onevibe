...
- [ ] **P4-02** Migrate database — Drizzle/PostgreSQL schema contract, six migrations, owner-bound conversation/task-lineage/provider-message/MCP-history/legacy-provenance tables, isolated async chat and project/task/schedule metadata repositories, owner-required importer, organization/member staging, cross-owner relationship validation, optimistic-conflict/restart proofs, disposable PostgreSQL migration/chat/metadata smoke proofs, and a fail-closed persistence-driver guard are now present; keep open until the full TaskStore repository adapter, production migration procedure, complete-surface idempotency proof, and a controlled `DATABASE_URL` runtime switch are complete
