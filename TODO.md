...
- [ ] **P4-02** Migrate database — Drizzle/PostgreSQL schema contract and initial migration now cover Better Auth, owner/org scope, tasks, turns, messages, events, leases, schedules, workspace versions, and MCP state; keep open until the TaskStore repository adapter, legacy import, Postgres restart/idempotency proof, and `DATABASE_URL` runtime switch are complete
