...
P4-02 progress note: the opt-in TaskStore now proves standalone messages, atomic native projections, transaction-backed fork history, and transaction-backed workspace byte/version restore/compare/copy recovery in addition to the operational Postgres surfaces. The remaining blockers are project-file bytes/revisions, attachments/previews/screenshots, full read/import coverage, and production runtime selection.
