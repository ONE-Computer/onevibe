...
- [ ] **P4-02** Migrate database — Drizzle/PostgreSQL schema contract, seven migrations, owner-bound conversation/task-lineage/provider-message/MCP-history/legacy-provenance tables, isolated async chat, metadata, and operational repositories, owner-required importer, organization/member staging, cross-owner relationship validation, optimistic-conflict/restart proofs, MCP audit retention, lease fencing, and disposable PostgreSQL migration/chat/metadata/operations smoke proofs are now present; keep open until the full TaskStore repository adapter, production migration procedure, complete-surface idempotency proof, and a controlled `DATABASE_URL` runtime switch are complete
