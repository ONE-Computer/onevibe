...
- [ ] **P4-02** Migrate database — Drizzle/PostgreSQL schema contract, four migrations, owner-required importer, cross-owner relationship validation, disposable live import/restart proof, a disposable PostgreSQL migration smoke proof, and a fail-closed persistence-driver guard are now present; keep open until the TaskStore repository adapter, production legacy import, application-level idempotency proof, and a controlled `DATABASE_URL` runtime switch are complete
