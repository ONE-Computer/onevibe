...
- [ ] **P4-02** Migrate database — Drizzle/PostgreSQL schema contract, seven migrations, owner-bound conversation/task-lineage/provider-message/MCP-history/legacy-provenance tables, isolated async chat, metadata, and operational repositories, owner-required importer, organization/member staging, cross-owner relationship validation, optimistic-conflict/restart proofs, MCP audit retention, lease fencing, native-event projection proofs, and an opt-in core `TaskStore` slice (`npm run e2e:postgres-taskstore`) are now present; the composed coordinator and opt-in TaskStore cover owner-checked MCP/org/skill/lease operations, standalone messages, and atomic native-envelope/projection ingestion plus restart recovery; keep open until workspace/project-file bytes have a durable repository, fork/read/import surfaces are complete, complete-surface idempotency/concurrency evidence exists, and a controlled `DATABASE_URL` runtime switch is safe
...
P4-02 progress note: the opt-in TaskStore now proves standalone messages, atomic native projections, and transaction-backed fork history in addition to the operational Postgres surfaces. The remaining blockers are durable workspace/project-file bytes, full read/import coverage, and production runtime selection.
