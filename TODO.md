...
- [ ] **P4-01** Add auth — feature-gated Better Auth + hashed email-OTP foundation, real delivery webhook, session middleware, login UI, expanded local cross-user route negative coverage, and a Drizzle/Postgres adapter proof (`npm run e2e:auth-postgres`) are implemented; keep open until the running Postgres TaskStore selects the same auth database, production delivery is accepted, and all data-plane ownership routes use Postgres-backed scope
- [ ] **P4-02** Migrate database — Drizzle/PostgreSQL schema contract, seven migrations, owner-bound conversation/task-lineage/provider-message/MCP-history/legacy-provenance tables, isolated async chat, metadata, and operational repositories, owner-required importer, organization/member staging, cross-owner relationship validation, optimistic-conflict/restart proofs, MCP audit retention, lease fencing, native-event projection proofs, transaction-backed fork history, and an opt-in core `TaskStore` slice (`npm run e2e:postgres-taskstore`) are now present; the composed coordinator and opt-in TaskStore cover owner-checked MCP/org/skill/lease operations, standalone messages, atomic native-envelope/projection ingestion, workspace bytes, immutable versions, compare/restore/copy, current project-file bytes/metadata, interrupted-task reconciliation, and restart recovery; keep open until project-file revision bytes, attachments/previews/screenshots, remaining read/import surfaces, per-tool-call write atomicity, complete-surface idempotency/concurrency evidence, and a controlled `DATABASE_URL` runtime switch are safe
...
P4-02 progress note: the opt-in TaskStore now proves standalone messages, atomic native projections, transaction-backed fork history, transaction-backed workspace byte/version restore/compare/copy recovery, current project-file update/restart recovery, and interrupted-task reconciliation in addition to the operational Postgres surfaces. The remaining blockers are project-file revision bytes, attachments/previews/screenshots, full read/import coverage, per-tool-call write atomicity, and production runtime selection.
