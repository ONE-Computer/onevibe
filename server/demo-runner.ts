  ...
  protected async execute({ task, store, signal, prompt, requestUserInput }: LegacyRuntimeContext) {
    ...
    if (task.mode === 'chat') {
      ...
      await store.appendEvent(task.id, {
        ...
        content: 'Simulation only: this response was not generated by a model. Select Claude Agent SDK for a real provider-backed conversation.',
        payload: { executionRoute: 'local_demo', simulated: true, modelCall: false },
      })
      await store.appendEvent(task.id, {
        type: 'run_completed', lane: 'control', status: 'completed', label: 'Simulation completed',
        content: 'The deterministic simulation completed without creating files or claiming provider execution.',
        payload: { executionRoute: 'local_demo', simulated: true, modelCall: false, filesCreated: 0, evidenceChainValid: store.verifyChain(task.id) },
      })
