...
type TaskRow = {
  ...
  priority: string | null; labels_json: unknown; assigned_agent: string | null; epic_id: string | null; epic_label: string | null;
...
const taskValues = (task: Task) => ({
  ...
  priority: task.priority ?? null, labels: json(task.labels ?? []), assignedAgent: task.assignedAgent ?? null, epicId: task.epicId ?? null, epicLabel: task.epicLabel ?? null,
  ...
  async load(ownerUserId?: string): Promise<{ projects: Project[]; tasks: Task[]; schedules: TaskSchedule[] }> {
    ...
    const tasks = await this.sql<TaskRow[]>`
      SELECT id, owner_user_id, org_id, conversation_id, project_id, title, prompt, provider, mode, status, skills_json, tags_json,
        priority, labels_json, assigned_agent, epic_id, epic_label,
    ...
    await this.sql.begin(async (tx) => {
      ...
      await tx`
        INSERT INTO task (
          ...
          ${values.skills}::jsonb, ${values.tags}::jsonb, ${values.priority}, ${values.labels}::jsonb, ${values.assignedAgent}, ${values.epicId}, ${values.epicLabel}, ${values.queuedGuidance}::jsonb, ${values.references}::jsonb, ${values.attachments}::jsonb,
  ...
  async updateTask(task: Task, expectedUpdatedAt: string): Promise<void> {
    ...
    const result = await this.sql`
      UPDATE task SET title = ${values.title}, prompt = ${values.prompt}, provider = ${values.provider}, mode = ${values.mode}, status = ${values.status},
        skills_json = ${values.skills}::jsonb, tags_json = ${values.tags}::jsonb, priority = ${values.priority}, labels_json = ${values.labels}::jsonb, assigned_agent = ${values.assignedAgent}, epic_id = ${values.epicId}, epic_label = ${values.epicLabel}, queued_guidance_json = ${values.queuedGuidance}::jsonb,
