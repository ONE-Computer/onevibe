...
describe('runtime readiness', () => {
  it('never presents unconfigured remote or sandbox runtimes as available', () => {
    const states = runtimeReadiness({ remoteConfigured: false, oneComputerConfigured: false }).providers
    expect(states.find((state) => state.id === 'demo')?.available).toBe(true)
    expect(states.find((state) => state.id === 'claude_sdk')?.available).toBe(true)
  ...
  it('reports configured boundaries without credential material', () => {
    const states = runtimeReadiness({ remoteConfigured: true, oneComputerConfigured: true }).providers
