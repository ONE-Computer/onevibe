...
export const portableArtifactKind = (artifactPath: string) => {
  ...
  if (normalized.startsWith('inputs/') || normalized.startsWith('evidence/') || normalized.startsWith('.claude/') || normalized.startsWith('.claude-state/') || normalized.startsWith('node_modules/') || normalized.includes('/node_modules/') || normalized.startsWith('.onevibe-')) return undefined
