...
export const toAssistantMessage = (message: AssistantConversationMessage): ThreadMessageLike => ({
  ...
  metadata: { custom: { taskId: message.taskId, turnId: message.turnId, provider: message.provider, inputFiles: message.inputFiles ?? [], artifacts: message.artifacts ?? [], trace: message.trace ?? [] } },
})
