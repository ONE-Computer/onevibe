...
export type AppView = 'agent' | 'schedules' | 'skills' | 'library' | 'computers' | 'appearance' | 'homepage' | 'artefacts' | 'capabilities' | 'board'

export const viewFromLocation = (): AppView => {
  if (typeof window === 'undefined') return 'agent'
  const value = new URLSearchParams(window.location.search).get('view')
  return value === 'schedules' || value === 'skills' || value === 'library' || value === 'computers' || value === 'appearance' || value === 'homepage' || value === 'artefacts' || value === 'capabilities' || value === 'board' ? value : 'agent'
}
