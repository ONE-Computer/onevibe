import { Activity, ArrowLeft, ArrowRight, Bot, Check, CheckCircle2, ClipboardCheck, Code2, Copy, Download, Eye, File, Files, GitFork, Globe2, History, Image, LoaderCircle, Maximize2, Minimize2, Network, Palette, Pencil, Presentation, RefreshCw, Save, Search, Settings2, ShieldCheck, Table2, TerminalSquare, TriangleAlert, User } from 'lucide-react'
...
import { providerLabel, statusLabel, tokenLabel } from '../lib/runtime-labels'
import { HUMAN_ASSIGNEE, parseAssignees, toggleAssignee } from '../lib/assignees'
...
export const Workspace = ({ task, projects, runtime, onMoveProject, onUpdateTags, onAssignAgent }: { task: TaskSnapshot; projects: Project[]; runtime?: RuntimeReadiness; onMoveProject: (taskId: string, projectId: string) => Promise<void>; onUpdateTags: (taskId: string, tags: string[]) => Promise<void>; onAssignAgent?: (taskId: string, assignedAgent: string | null) => Promise<void> }) => {
  ...
  const assignees = useMemo(() => parseAssignees(task.assignedAgent), [task.assignedAgent])
  const assigneeOptions = useMemo(() => {
    const options = (runtime?.providers ?? []).map((provider) => ({ id: provider.id as string, label: provider.label }))
    const known = new Set(options.map((option) => option.id))
    for (const assignee of assignees) {
      if (assignee !== HUMAN_ASSIGNEE && !known.has(assignee)) {
        options.push({ id: assignee, label: assignee })
        known.add(assignee)
      }
    }
    options.push({ id: HUMAN_ASSIGNEE, label: 'Human' })
    return options
  }, [runtime?.providers, assignees])
  const saveAssigneeToggle = (id: string) => {
    const assignedAgent = toggleAssignee(task.assignedAgent, id) ?? null
    if (assignedAgent === (task.assignedAgent ?? null)) return
    setSavingAgent(true)
    setAgentError(null)
    void onAssignAgent?.(task.id, assignedAgent).catch((error: unknown) => setAgentError(error instanceof Error ? error.message : 'Unable to assign agent')).finally(() => setSavingAgent(false))
  }
    ...
    <section className={`workspace secure-signal-cut ${fullscreen ? 'workspace-fullscreen' : ''}`}>
      ...
      <div className="workspace-body">
        <AnimatePresence mode="wait">
          ...
          {tab === 'assets' && <motion.div key="assets" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="assets-pane"><header><div><Image size={17} /><div><span>Portable visual artifacts</span><strong>{assetFiles.length} generated asset{assetFiles.length === 1 ? '' : 's'}</strong></div></div><small>Path-confined workspace files</small></header><div>{assetFiles.map((file) => <article key={file.path}><img src={`/api/tasks/${task.id}/file?path=${encodeURIComponent(file.path)}&raw=1`} alt={file.path} /><footer><div><strong>{file.path}</strong><small>{readableBytes(file.size)}</small></div><a href={`/api/tasks/${task.id}/file?path=${encodeURIComponent(file.path)}&download=1`} title={`Download ${file.path}`}><Download size={13} /></a></footer></article>)}</div></motion.div>}
          {tab === 'visual' && (
          ...
          {tab === 'files' && (
            <motion.div key="files" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="files-pane">
              ...
              {files.map((file) => isBinary(file.path) ? <a className="file-download-row" key={file.path} href={`/api/tasks/${task.id}/file?path=${encodeURIComponent(file.path)}&download=1`}><Download size={15} /><span><strong>{file.path}</strong><small>Download binary artifact</small></span><em>{readableBytes(file.size)}</em></a> : <button key={file.path} onClick={() => { setSelectedFile(file.path); setTab('code') }}><File size={15} /><span><strong>{file.path}</strong><time dateTime={file.updatedAt}>{new Date(file.updatedAt).toLocaleTimeString()}</time></span><em>{readableBytes(file.size)}</em></button>)}
            </motion.div>
          )}
          {tab === 'history' && (
          ...
          {tab === 'settings' && (
            <motion.div key="settings" initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} className="task-settings-pane">
              ...
              <article><span>Runtime</span><strong>{providerLabel(task.provider)}</strong><small>{task.securityContext?.executionBoundary ? tokenLabel(task.securityContext.executionBoundary) : 'Awaiting run boundary'}</small></article>
