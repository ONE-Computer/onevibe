...
import type { ConversationSummary, Project, ProjectFileVersion, Task, TaskMode } from '../types'
...
export const Sidebar = ({ view, conversations, tasks = [], activeTaskId, onNewTask, onClose, onSelectTask, hasMoreConversations, loadingMoreConversations, onLoadMoreConversations, projects, activeProjectId, onSelectProject, onCreateProject, onAttachProjectFile, onRemoveProjectFile, onUpdateProjectFile, onRestoreProjectFile, onUpdateProjectContext, onOpenSkills, onOpenLibrary, onOpenSchedules, onOpenComputers, onOpenBoard, onOpenAppearance, onOpenHomepage, onOpenArtefacts, onOpenCapabilities, skillCount, user, onSignOut, locale = 'en' }: Props) => {
  ...
  <aside className="sidebar">
    ...
    <nav className="primary-nav" aria-label="Primary">
      <button className={`nav-item ${view === 'agent' ? 'active' : ''}`} aria-current={view === 'agent' ? 'page' : undefined} onClick={onNewTask}><Sparkles size={16} /> Agent</button>
      <button className={`nav-item ${view === 'skills' ? 'active' : ''}`} aria-current={view === 'skills' ? 'page' : undefined} onClick={onOpenSkills}><Blocks size={16} /> {t('skills', locale)} <span className="nav-pill">{skillCount}</span></button>
      <button className={`nav-item ${view === 'schedules' ? 'active' : ''}`} aria-current={view === 'schedules' ? 'page' : undefined} onClick={onOpenSchedules}><Clock3 size={16} /> {t('scheduled', locale)}</button>
      <button className={`nav-item ${view === 'library' ? 'active' : ''}`} aria-current={view === 'library' ? 'page' : undefined} onClick={onOpenLibrary}><Library size={16} /> {t('library', locale)}</button>
      <button className={`nav-item ${view === 'computers' ? 'active' : ''}`} aria-current={view === 'computers' ? 'page' : undefined} onClick={onOpenComputers}><MonitorCog size={16} /> {t('computers', locale)}</button>
      <button className={`nav-item ${view === 'board' ? 'active' : ''}`} aria-current={view === 'board' ? 'page' : undefined} onClick={onOpenBoard}><Kanban size={16} /> {t('board', locale)}</button>
      <button className={`nav-item ${view === 'appearance' ? 'active' : ''}`} aria-current={view === 'appearance' ? 'page' : undefined} onClick={onOpenAppearance}><Palette size={16} /> {t('appearance', locale)}</button>
      <button className={`nav-item ${view === 'homepage' ? 'active' : ''}`} aria-current={view === 'homepage' ? 'page' : undefined} onClick={onOpenHomepage}><FileEdit size={16} /> Homepage</button>
      <button className={`nav-item ${view === 'artefacts' ? 'active' : ''}`} aria-current={view === 'artefacts' ? 'page' : undefined} onClick={onOpenArtefacts}><FolderOpen size={16} /> Artefacts</button>
      <button className={`nav-item ${view === 'capabilities' ? 'active' : ''}`} aria-current={view === 'capabilities' ? 'page' : undefined} onClick={onOpenCapabilities}><Zap size={16} /> Capabilities</button>
    ...
    {projects.map((project) => <button key={project.id} className={`project-row ${project.id === activeProjectId ? 'selected' : ''}`} aria-current={project.id === activeProjectId ? 'true' : undefined} onClick={() => onSelectProject(project.id)}><FolderKanban size={14} /> <span>{project.name}</span></button>)}
    ...
    <ActiveNowPanel tasks={tasks} projects={projects} locale={locale} variant="sidebar" onOpenTask={onSelectTask} />
          ...
          {group.items.map((conversation) => {
            ...
            return (
              <motion.button
                layout
                key={conversation.id}
                className={`task-row ${activeTaskId === conversation.id ? 'selected' : ''} ${conversation.status === 'running' ? 'working' : ''}`}
                aria-current={activeTaskId === conversation.id ? 'true' : undefined}
