...
export const Library = ({ items, projects, onOpenTask }: Props) => {
  ...
  return <section className="library-view">
    <header><div><span className="task-kicker">Your artifacts</span><h1>Library</h1><p>Completed artifacts across projects. Reopen the originating task to inspect the conversation, evidence chain, source, and any approval history.</p></div><LibraryBig size={28} /></header>
    {!items.length ? <div className="library-empty"><Box size={22} /><strong>No completed artifacts yet</strong><span>Finished work will appear here with its original task and evidence trail.</span></div> : <><div className="library-retrieval"><label><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, project, tag, mode, or artifact path" aria-label="Search library artifacts" />{query && <button onClick={() => setQuery('')} aria-label="Clear library search"><X size={13} /></button>}</label><div>{(['all', ...modes] as const).map((candidate) => <button key={candidate} className={mode === candidate ? 'active' : ''} onClick={() => setMode(candidate)}>{candidate === 'all' ? `All ${items.length}` : typeLabel(candidate)}</button>)}</div>{tags.length > 0 && <div className="library-tag-filter">{(['all', ...tags] as const).map((candidate) => <button key={candidate} className={tag === candidate ? 'active' : ''} onClick={() => setTag(candidate)}>{candidate === 'all' ? 'All tags' : candidate}</button>)}</div>}</div>{!filtered.length ? <div className="library-empty library-no-results"><Search size={22} /><strong>No matching artifacts</strong><span>Try a different task name, project, tag, mode, or file path.</span></div> : <div className="library-grid">{filtered.map(({ task, files }) => <motion.article layout key={task.id}><div className="library-card-top"><span>{typeLabel(task.mode)}</span><time dateTime={task.updatedAt}>{new Date(task.updatedAt).toLocaleDateString()}</time></div><h2>{task.title}</h2><p>{projects.find((project) => project.id === task.projectId)?.name ?? 'Project workspace'}</p>{task.tags.length > 0 && <div className="library-card-tags">{task.tags.map((item) => <span key={item}>{item}</span>)}</div>}<div className="library-files"><FolderOpen size={13} /> {files.length} artifact{files.length === 1 ? '' : 's'}{files.slice(0, 2).map((file) => <small key={file.path}><FileCode2 size={11} /> {file.path}</small>)}</div><button aria-label={`Open ${task.title}`} onClick={() => onOpenTask(task.id)}>Open task <ExternalLink size={13} /></button></motion.article>)}</div>}</>}
  </section>
}
