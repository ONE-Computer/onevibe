...
export const Schedules = ({ schedules, activeProjectId, onCreate, onToggle, onRunNow, runtime }: Props) => {
  ...
  return <section className="schedules-view">
    ...
    <div className="schedule-list">{schedules.length === 0 ? <p>No scheduled work yet — create one above to run a task on a repeating cadence.</p> : schedules.map((schedule) => <article key={schedule.id}><div><strong>{schedule.name}</strong><span>{schedule.prompt}</span><small>{schedule.provider === 'demo' ? 'Safe demo' : schedule.provider === 'claude_sdk' ? 'Claude Agent SDK' : schedule.provider === 'onecomputer' ? 'ONEComputer sandbox' : 'Remote runtime'} · {schedule.intervalMinutes >= 1440 ? `Every ${schedule.intervalMinutes / 1440} day${schedule.intervalMinutes === 1440 ? '' : 's'}` : `Every ${schedule.intervalMinutes} minutes`} · next {new Date(schedule.nextRunAt).toLocaleString()}</small></div><aside>{schedule.enabled && <button onClick={() => void onRunNow(schedule)}><Zap size={13} /> Run now</button>}<button onClick={() => void onToggle(schedule)}>{schedule.enabled ? <><Pause size={13} /> Pause</> : <><Play size={13} /> Resume</>}</button></aside></article>)}</div>
  </section>
}
