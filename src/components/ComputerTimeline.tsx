...
import { artifactRailItems, causalVisualItemsFor, defaultComputerItem, evidenceItemId, filterItemsByRun, formatDuration, formatInspectable, matchesRailQuery, presentationItems, runIdsFor, summarizeRunEvidence, terminalActivityFor, virtualRailRange, type ComputerItem } from './computer-timeline-activity'
...
export const ComputerTimeline = ({ task }: { task: TaskSnapshot }) => {
  ...
  const comparison = useMemo(() => comparisonRunId && runFilter !== 'all' && comparisonRunId !== runFilter ? { baseline: summarizeRunEvidence(railItems, comparisonRunId), candidate: summarizeRunEvidence(railItems, runFilter) } : undefined, [comparisonRunId, railItems, runFilter])
