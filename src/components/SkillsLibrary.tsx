...
export const SkillsLibrary = ({ catalog = fallbackSkillCatalog, selected, onToggle }: Props) => <section className="skills-view">
  <header><div><span className="task-kicker">Capability guides</span><h1>Skills</h1><p>Choose up to four working guides for new tasks. Skills shape the agent’s method and are captured in evidence; they do not grant tools, data access, or approval authority.</p></div><Sparkles size={28} /></header>
  <div className="skills-grid">{catalog.map((skill) => { const Icon = icons[skill.id] ?? Sparkles; const active = selected.includes(skill.id); const locked = !active && selected.length >= 4; return <motion.button layout key={skill.id} className={active ? 'active' : ''} onClick={() => onToggle(skill.id)} disabled={locked} title={locked ? 'Remove a selected guide before adding another' : undefined} aria-disabled={locked}><Icon size={18} /><div><strong>{skill.title}</strong><span>{skill.summary}</span></div>{active && <Check size={16} />}</motion.button> })}</div>
