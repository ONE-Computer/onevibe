...
import type { BoardStatus, ConversationSummary, LibraryItem, Project, RuntimeMcpConfig, Task, TaskAttachment, TaskMode, TaskPriority, TaskSchedule, TaskSkill } from './types'
import './index.css'
...
export default function App() {
  ...
  const { data: tasksData, error: tasksError, refetch: refetchTasks } = useQuery({ queryKey: ['tasks'], queryFn: listTasks, staleTime: 5_000, refetchOnWindowFocus: false, refetchInterval: (query) => query.state.data?.tasks.some((task) => ['pending', 'running', 'waiting_for_approval', 'waiting_for_user_input'].includes(task.status)) ? 5_000 : false })
  ...
  const themeAdminQuery = useQuery({ queryKey: ['theme', 'admin'], queryFn: listTenantThemes, enabled: !authLoading && (!authState?.enabled || Boolean(authState.session)), staleTime: 30_000, retry: false })
  ...
  const handlePatchTask = useCallback(async (taskId: string, patch: { status?: BoardStatus | null; priority?: TaskPriority | null }) => {
    try {
      const updated = await patchTask(taskId, patch)
      updateTasksCache((current) => current.map((task) => (task.id === updated.id ? { ...task, ...updated } : task)))
    } catch (reason) {
      reportError(reason, 'Unable to update task')
    }
  }, [updateTasksCache])
  ...
  return (
    ...
    <>
      ...
      <AnimatePresence>{sidebarOpen && <><motion.button key="sidebar-backdrop" className="sidebar-backdrop" aria-label="Close sidebar" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSidebarOpen(false)} /><motion.div key="sidebar-panel" initial={{ x: -260 }} animate={{ x: 0 }} exit={{ x: -260 }}><Sidebar view={view} conversations={conversations} tasks={tasks} activeTaskId={activeTaskId} onNewTask={() => navigateToTask(null)} onClose={() => setSidebarOpen(false)} onSelectTask={(taskId) => navigateToTask(taskId)} hasMoreConversations={Boolean(conversationsHasNextPage)} loadingMoreConversations={conversationsIsFetchingNextPage} onLoadMoreConversations={loadMoreConversations} projects={projects} activeProjectId={activeProjectId} onSelectProject={setActiveProjectId} onCreateProject={addProject} onAttachProjectFile={attachProjectFile} onRemoveProjectFile={detachProjectFile} onUpdateProjectFile={editProjectFile} onRestoreProjectFile={restoreProjectFile} onUpdateProjectContext={updateProject} onOpenSkills={() => navigateToView('skills')} onOpenLibrary={() => navigateToView('library')} onOpenSchedules={() => navigateToView('schedules')} onOpenComputers={() => navigateToView('computers')} onOpenBoard={() => navigateToView('board')} onOpenAppearance={() => navigateToView('appearance')} onOpenHomepage={() => navigateToView('homepage')} onOpenArtefacts={() => navigateToView('artefacts')} onOpenCapabilities={() => navigateToView('capabilities')} skillCount={skillCatalog.length} user={authState?.session?.user} onSignOut={signOut} locale={locale} /></motion.div></>}</AnimatePresence>
