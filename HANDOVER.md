...
`THEMING_EXTENSIBILITY.md` is a planning brief for a future white-label product surface. `TODO.md` now tracks P7-01 through P7-09. The local P7-03 Postgres persistence/API slice and initial P7-04 ThemeProvider projection are implemented and covered by the authenticated Postgres harness; reference seed tooling, admin controls, full tenant isolation, and production deployment remain open. Theme configuration is presentation-only: it cannot change LiteLLM routing, provider credentials, auth/session policy, OpenVTC/VTI approval authority, evidence redaction, or sandbox policy. The current ONEVibe UI remains sans-serif-only; the brief's serif/monospace examples require an explicit design/security decision before they could be considered.
