...
3. **The running app is not Postgres-backed** — local user/org scoping is proven, and the Drizzle schema/import/restart proof now stages organization/member records as well; migrations `0004` and `0005` bring conversation identity, owner binding, task lineage, provider message IDs, MCP config history, lease idempotency uniqueness, legacy-import provenance, and the task-to-conversation FK into the target contract. Isolated async Postgres chat and metadata repositories now prove owner-scoped task/project/schedule persistence, optimistic conflicts, and restart reload, but the full TaskStore repository adapter, production importer procedure, complete-surface idempotency proof, and `DATABASE_URL` runtime switch remain Phase 4 work
