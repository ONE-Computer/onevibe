...
> **Date**: 2026-07-17
> **Status**: Local phases 1–3 are implemented and the professional UI has crossed the Zustand and ordinary-collection TanStack Query boundaries. Governed MCP declarations, bounded MCP health probing, a tested opt-in MCP capability facade, truthful demo skill status, a GitHub-catalog skill marketplace with protected Claude/LiteLLM materialization evidence, feature-gated Better Auth, authenticated owner scoping, a local organization-membership scaffold, and a reviewed Drizzle/Postgres import proof (including organization/member staging) are present. The target schema now also records durable conversation identity, fork lineage, provider message IDs, MCP configuration history, lease idempotency keys, and legacy-import provenance. LiteLLM-only enforcement remains mandatory for every harness and every environment: data sovereignty, centralized routing, cost control, and model optimization are product requirements. A composed `PostgresStateCoordinator` now has a disposable restart proof across metadata, messages, runtime events, native envelopes, projection links, offsets, replay/conflict handling, owner isolation, operational wrappers, and workspace bytes/version snapshots; an opt-in core `TaskStore` Postgres slice now routes operational MCP/org/skill/lease surfaces, project/task/chat/event-chain/retry persistence, transaction-backed fork history, workspace restore/compare/copy, current project-file bytes/metadata, and interrupted-task reconciliation. The running application is still SQLite-backed; project-file revision bytes, attachments/previews/screenshots, the remaining synchronous read/import surface, per-tool-call runtime write atomicity, full async TaskStore/server integration, production auth, the Postgres repository/runtime switch, deployment, cloud sandbox attestation, MCP secret brokering, and external MCP health/attestation remain open.
...
2. **Auth is feature-gated** — Better Auth Email OTP, session middleware, login UI, local user ownership, and a Drizzle/Postgres OTP/session proof are implemented, but production enablement remains blocked on selecting the same Postgres database from the running TaskStore/server, org scope, real delivery, and full route acceptance
3. **The running app is not Postgres-backed** — local user/org scoping is proven, and the Drizzle schema/import/restart proof now stages organization/member records as well; migrations `0004`–`0006` bring conversation identity, owner binding, task lineage, provider message IDs, MCP config history/retention, lease idempotency uniqueness, legacy-import provenance, and the task-to-conversation FK into the target contract. Isolated async Postgres chat, metadata, and operational repositories now prove owner-scoped task/project/schedule persistence, MCP audit retention, organizations, skills, lease fencing, idempotency, optimistic conflicts, and restart reload, but the full TaskStore repository adapter, production importer procedure, complete-surface idempotency proof, and `DATABASE_URL` runtime switch remain Phase 4 work
...
BETTER_AUTH_SECRET=
RESEND_API_KEY=
# DATABASE_URL=postgres://...  # application startup rejects this until P4-02 is wired
E2B_API_KEY=
