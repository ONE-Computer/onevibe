...
> **Date**: 2026-07-17
> **Status**: Local phases 1–3 are implemented and the professional UI has crossed the Zustand and ordinary-collection TanStack Query boundaries. Governed MCP declarations, bounded MCP health probing, a tested opt-in MCP capability facade, truthful demo skill status, a GitHub-catalog skill marketplace with protected Claude/LiteLLM materialization evidence, feature-gated Better Auth, authenticated owner scoping, a local organization-membership scaffold, and a reviewed Drizzle/Postgres import proof (including organization/member staging) are present. The target schema now also records durable conversation identity, fork lineage, provider message IDs, MCP configuration history, lease idempotency keys, legacy-import provenance, workspace bytes, and project revision bytes. LiteLLM-only enforcement remains mandatory for every harness and every environment: data sovereignty, centralized routing, cost control, and model optimization are product requirements. The running server can now opt into the owner-scoped Postgres TaskStore and matching Better Auth database; HTTP read refresh and unauthenticated owner-scope rejection are proven. Remaining P4 gaps are attachment/import/export byte round trips, complete workflow idempotency/concurrency, cross-instance live SSE, production email/deployment, cloud sandbox attestation, MCP secret brokering, and external MCP health/attestation.
