...
The Postgres target contract lives in `server/db/schema.ts` and `server/db/migrations/`; run `npm run db:check` and regenerate only through the reviewed Drizzle config. `npm run db:import -- --dry-run` is the safe first inspection of legacy data; the write path requires `DATABASE_URL`, an existing Better Auth owner, and explicit owner assignment for ownerless records. `server/persistence/postgres-chat.ts`, `postgres-metadata.ts`, and `postgres-operations.ts` plus `npm run e2e:postgres-chat`, `npm run e2e:postgres-metadata`, and `npm run e2e:postgres-operations` are isolated async proofs for the conversation, metadata, and operational slices. `PostgresStateCoordinator` composes those repositories; `npm run e2e:postgres-state` now proves restart recovery across metadata, messages, runtime events, native envelopes, projection links, offsets, replay/conflict handling, and owner isolation. It is still an integration seam rather than the active TaskStore driver: native ingestion is not yet one cross-repository transaction and the synchronous TaskStore/server surface remains open. Do not flip `DATABASE_URL` into production behavior until the full TaskStore repository adapter, transaction boundaries, restart/idempotency tests, and deployment migration procedure exist. A passing schema generator, import dry-run, or isolated/coordinated repository proof is not evidence that the application is using Postgres.
