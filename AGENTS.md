...
The Postgres target contract lives in `server/db/schema.ts` and `server/db/migrations/`; run `npm run db:check` and regenerate only through the reviewed Drizzle config. Do not flip `DATABASE_URL` into production behavior until the TaskStore repository adapter, owner-aware legacy import, transaction boundaries, restart/idempotency tests, and deployment migration procedure exist. A passing schema generator is not evidence that the application is using Postgres.
