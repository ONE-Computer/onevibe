...
The Better Auth foundation is feature-gated by `ONEVIBE_AUTH_ENABLED=true`. It must fail closed when `BETTER_AUTH_SECRET` or the real OTP webhook is missing. Do not enable it in a deployed environment until every data-plane repository accepts the server-derived user/org scope; the current implementation returns `auth_ownership_not_ready` instead of exposing the legacy global SQLite store to an authenticated session. Never add a fake OTP, browser-returned OTP, or development console OTP to make the login appear complete.
