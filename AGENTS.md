...
The Postgres target contract lives in `server/db/schema.ts` and `server/db/migrations/`; run `npm run db:check` and regenerate only through the reviewed Drizzle config. `npm run db:import -- --dry-run` is the safe first inspection of legacy data; the write path requires `DATABASE_URL`, an existing Better Auth owner, and explicit owner assignment for ownerless records. `server/persistence/postgres-chat.ts` and `npm run e2e:postgres-chat` are the isolated async chat proof; they are not the active TaskStore driver. Do not flip `DATABASE_URL` into production behavior until the full TaskStore repository adapter, transaction boundaries, restart/idempotency tests, and deployment migration procedure exist. A passing schema generator, import dry-run, or isolated chat proof is not evidence that the application is using Postgres.
