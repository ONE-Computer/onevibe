...
3. **P0c — replay (in progress):** Selection, scroll anchor, filters, and deep links survive reload/reconnect; the rail can play preserved visual evidence in chronological order, then pause for manual immutable inspection. Multi-run tasks surface visible run chapters and can be filtered to one immutable run through a deep-linkable selector. Immutable workspace versions can now be compared against the current workspace through bounded path/hash metadata without copying source into evidence. Add prior-run comparison and synchronized production browser/desktop playback without mutating evidence.
