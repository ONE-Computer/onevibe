...
4. **P0d — browser evidence (in progress):** Website, App, and Game outputs now add an explicit sandbox-local `file://` Chromium screenshot after artifact extraction, with hostname resolution blocked. Next, capture safe browser-tool checkpoints during the agent loop—not only X11 checkpoints and the final generated-artifact render—and attach each to its tool-use ID.
