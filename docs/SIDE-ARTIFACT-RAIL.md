...
3. **P0c — replay (in progress):** Selection, scroll anchor, filters, and deep links survive reload/reconnect; the rail can play preserved visual evidence in chronological order, then pause for manual immutable inspection. Immutable workspace versions can now be compared against the current workspace through bounded path/hash metadata without copying source into evidence. Add explicit run chapters, prior-run comparison, and synchronized production browser/desktop playback without mutating evidence.
