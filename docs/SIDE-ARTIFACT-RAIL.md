...
3. **P0c — replay (in progress):** Selection, scroll anchor, filters, and deep links survive reload/reconnect; the rail can play preserved visual evidence in chronological order, then pause for manual immutable inspection. A visible range scrubber moves across the same chronological event selection as keyboard stepping, exits live follow, and keeps the selected card in the virtualized rail—without creating a second playback state. Filter and comparison state is immediately URL-addressable, even before another card is selected. Multi-run tasks surface visible run chapters and can be filtered to one immutable run through a deep-linkable selector; two runs can now be compared through bounded evidence counts, elapsed spans, and added/removed projected deliverable identifiers. Historical tasks without persisted run IDs derive display-only `Turn 1`, `Turn 2` boundaries from their immutable `run_started` events—without rewriting the ledger. Immutable workspace versions can now be compared against the current workspace through bounded path/hash metadata without copying source into evidence. Add richer prior-run artifact comparison and synchronized production browser/desktop playback without mutating evidence.
