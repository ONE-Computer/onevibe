...
10. dependency review: `npm audit --omit=dev` currently reports five moderate advisories through the Better Auth → Drizzle Kit → esbuild dependency chain. The available automated fix is a breaking downgrade of `drizzle-kit`; do not run `npm audit fix --force` as a release action. Resolve or formally accept this risk before production deployment, with the development-server exposure bounded away from production traffic.
