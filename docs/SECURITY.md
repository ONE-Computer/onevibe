...
- Optional generated-project build validation is disabled by default and requires both `ONEVIBE_SANDBOX_BUILD_VALIDATION=true` and gateway attestation. It runs only inside the disposable sandbox; dependency lifecycle scripts are disabled during install, the browser receives only a bounded report, and a successful build is not treated as dependency-provenance or deployment proof.
