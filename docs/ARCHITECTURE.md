...
The current state boundary has three Zustand stores: `useUiStore` owns navigation, inspector, and connection presentation state; `useComposerStore` owns selected skill guides and submission state; `useSessionStore` owns the authenticated session probe. TanStack Query is mounted at the root and currently owns the cached Skills catalog, runtime-readiness, and MCP declaration queries; MCP mutations update that cache. Tasks, conversations, projects, schedules, and Library remain server-backed data held by the transitional App layer until the query/mutation migration is complete. The durable task SSE stream remains outside Query as an append-only event projection.
