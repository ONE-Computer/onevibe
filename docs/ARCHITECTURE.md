...
The Compose path intentionally does not pretend to use Postgres yet. `DATABASE_URL`, `better-auth`, user scoping, and a Postgres service belong to the still-open Phase 4 persistence/auth slices. The opt-in coordinator now stores task workspace bytes and immutable workspace snapshots in Postgres and hydrates the local filesystem as a materialized cache; project-file revisions, attachments/previews/screenshots, and the remaining TaskStore read/import surface are not yet promoted. Until those contracts land, cloud promotion must treat the SQLite volume as a single-instance local deployment boundary rather than a multi-user production database.
