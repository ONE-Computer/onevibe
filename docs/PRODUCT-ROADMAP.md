...
| **R1.1 P0 Side timeline (weeks 1–6)** | **Highest-priority interaction track:** an artifact-first mixed stream where the rail automatically alternates compact CLI command/result cards, causal X11/browser frames, and file/diff/preview/slide cards based on the agent's actual tool call—not a generic log; versioned typed presentation-event contract; live-follow with clear pause/resume; keyboard-accessible backward/forward stepping and time scrub; each panel bound to task, run, sequence, timestamp, artifact version, and evidence hash | Contract/schema tests across every panel type; real-session proof that command work renders command/output while graphical work renders a correlated capture; explicit no-frame state rather than fabricated imagery; reconnect/order/idempotency E2E; live-follow and scrub interaction tests; 10,000-event task opens p95 <2s and scrub response p95 <100ms on reference hardware; 100% rendered panels resolve to verifying evidence; no mutable historical panel |
