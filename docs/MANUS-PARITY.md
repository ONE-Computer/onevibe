...
4. **P** Live plan progress — live statuses exist and each continuation resets the durable plan lifecycle with evidence linking the prior run. Provider-native envelopes now persist with source cursors and versioned projections; richer provider-native plan/progress mapping remains pending.
...
6. **I** Narrated execution — typed transcript deltas from demo, SDK, or remote runtime. Claude SDK, ONEComputer journal, and remote frames now pass through durable redacted native envelopes before projection. A controlled opt-in live harness verifies the Claude SDK lifecycle, workspace artifact, and evidence chain only when server-side credential readiness is explicitly configured.
7. **P** Interruptible follow-up chat — resumable same-session follow-ups work after a turn; each ONEComputer conversation now owns a generation-fenced retained development sandbox for its continuations, while a different conversation receives a different boundary. Active non-interruptible provider turns accept bounded guidance queued for the next turn. Users can review and retract queued guidance before it reaches a provider; cancellation records metadata-only control evidence, not the guidance text. Users can stop a running, pending, input-waiting, or approval-waiting task while preserving workspace/evidence. True provider-native live interruption/injection remains pending.
...
15a. **P** Skill library — eight versioned, bundled task skill packs can be selected (up to four per task). Each turn records an immutable ID/version/SHA-256 manifest; native Claude SDK turns materialize only that pinned set under the task’s `.claude/skills` directory and enable it through the SDK’s native skill filter, while ONEComputer turns transfer the same selected files into the disposable sandbox. Third-party skill installation, organization policy packs, signed/imported bundles, scan/revocation workflows, and per-project defaults remain pending.
...
30. **P** Build Website mode — portable React/Vite output now includes a responsive, accessible enterprise landing-page starter with clear operating-boundary messaging, semantic FAQ disclosure, and a validated HTML shell/entry contract. A bounded `e2e:website-build -- --install` harness extracts a real demo artifact and compiles it to `dist/index.html` in a temporary directory. ONEVibe’s own task screen has desktop/390px browser evidence for a real completed Website task; generated-project browser/a11y testing, AI-directed design, richer template packs, and deployment remain pending.
...
67. **P** Database workspace tab — Data mode renders the portable generated CSV as an inspectable in-workspace table, uses a bounded quoted-field parser with visible malformed-input errors, exposes source-lineage metadata, supports bounded local row filtering (up to 500 visible matches), and can open the source. Filters do not mutate data or call a connector; live data sources, edits, and query tooling remain pending.
