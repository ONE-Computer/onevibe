...
15. **P** Reusable project context — a project now retains a governed brief plus up to twelve bounded, path-confined knowledge files. Text-like files are attached server-side as untrusted context with immutable metadata-only evidence, can be edited through an optimistic-hash-protected text editor, and stale files can be removed before they reach future tasks without altering the brief. Folder sync, fine-grained project permissions, connected drives, and file-version management remain pending.
