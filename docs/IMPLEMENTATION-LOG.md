...
- Made Claude run budgets transport-aware in commit `63e19bb`. Local LiteLLM defaults now use 12 turns and a $2 budget, matching the bounded slide proof; direct Anthropic keeps the larger 24-turn/$5 defaults. Both remain explicitly overridable and are clamped by typed parsing tests. Full check passes with 35 test files / 198 tests. This is a local reliability policy, not proof of model quality or provider availability.
