...
The Postgres migration work is tracked in [ONE-254](https://linear.app/onecomputer/issue/ONE-254/platp1-implement-postgres-drizzle-repository-and-migrationimport-proof). The current branch contains the reviewed Drizzle schema, two generated SQL migrations, an explicit owner-required `db:import` seam, and a fail-closed driver guard that rejects an unavailable Postgres selection or ambiguous `DATABASE_URL`. A disposable PostgreSQL 18 import/restart proof passed, but the running TaskStore remains SQLite-backed until transaction-compatible repositories, controlled runtime selection, and production idempotency/connection evidence are complete.
