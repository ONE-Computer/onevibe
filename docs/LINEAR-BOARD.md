...
ONE-220 local-provider clarification: commit `066909e` and diagnostic task `task_728f380252044d` show that the current host-process document-mode Claude/LiteLLM loop can reach governed plan/Bash/Read calls and then time out before `verify`/`deliver`. The existing sandbox-origin artifact evidence remains valid, but the local provider-backed document acceptance gate is open as P1-10; no synthetic artifact fallback is permitted.
