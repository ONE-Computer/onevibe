...
The Zustand state migration is tracked in [ONE-259](https://linear.app/onecomputer/issue/ONE-259/uxp1-migrate-application-state-to-explicit-zustand-stores). It is In Progress: the three explicit UI/composer/session stores are implemented and tested, while server-backed collection ownership remains open for the TanStack Query migration.

The TanStack Query migration is tracked in [ONE-260](https://linear.app/onecomputer/issue/ONE-260/uxp1-migrate-server-backed-collections-to-tanstack-query). It is In Progress: the root QueryClient and Skills catalog query are implemented; remaining collection queries/mutations must still be migrated without duplicating task SSE authority.
