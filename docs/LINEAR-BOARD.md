...
P7-03/P7-04 local follow-up: ONE-264 now has the Postgres-backed `tenant_theme_config` and append-only `tenant_theme_config_event` tables in migrations `0012`/`0013`, versioned reset tombstones, transactional current `org_member.role=owner` authorization, server-only `ONEVIBE_THEME_TENANT_ORG_MAP` creation binding, typed `theme_version_conflict` responses, and authenticated current/owner GET/PUT/reset APIs. SQLite intentionally has no theme authority. ONE-263 now has the server-authoritative React `ThemeProvider`/`ThemeSlot` projection with prefixed CSS variables, revision cleanup, and bounded logo URL checks. `npm run e2e:postgres-auth-http` proves base fallback, owner/member isolation, update conflict, reset, audit events, and browser tenant-id rejection; the browser evidence is `local-home-20260717-theme-provider.jpg`. Reference seed/import tooling, admin appearance/content UI, luminance/contrast/asset-integrity acceptance, and production org/deployment gates remain open.
