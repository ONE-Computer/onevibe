...
The Postgres migration work is tracked in [ONE-254](https://linear.app/onecomputer/issue/ONE-254/platp1-implement-postgres-drizzle-repository-and-migrationimport-proof). The current branch contains the reviewed Drizzle schema, six generated SQL migrations, an explicit owner-required `db:import` seam, owner-bound conversation/task identity, organization/member staging with Better Auth-user existence checks, explicit task-lineage/provider-message/MCP-history/legacy-provenance target tables, pre-transaction validation that rejects missing or cross-owner task/schedule project references, a disposable PostgreSQL 18 migration/import/restart proof, an isolated async owner-scoped chat repository with `npm run e2e:postgres-chat`, a CI PostgreSQL migration/chat smoke job, and a fail-closed driver guard that rejects an unavailable Postgres selection or ambiguous `DATABASE_URL`. Migration `0003` gives the native projection offset primary key an explicit non-truncated name while preserving the historical migration ledger; migrations `0004`–`0005` add the remaining durable identity/audit contract without switching the runtime driver. The running TaskStore remains SQLite-backed until the full transaction-compatible repository, controlled runtime selection, and production idempotency/connection evidence are complete.
