...
ONE-254 operation-journal follow-up: commit pending in the next push adds the durable `follow_up_operation` journal to SQLite/Postgres, idempotent attachment/guidance materialization, startup recovery for prepared/ready operations, and explicit failure for provider-claimed operations whose external outcome is unknown. `npm run e2e:follow-up-recovery` proves a development-only crash immediately after preparation, restart recovery of exactly one four-message follow-up with one attachment, and HTTP `200` replay without duplication. Remaining gaps are atomic attachment/task persistence, durable execution leases, provider request identity, and unknown-outcome reconciliation.
