...
ONE-254 / P4-02 follow-up: the latest local slice adds replay-safe TaskStore turn reservation. SQLite checks the durable turn repository before inserting; Postgres uses the existing unique client-request contract and skips duplicate assistant placeholders. Retry/follow-up paths pass their request key into the reservation, and terminal replays do not reactivate a run. `npm run check` passed with 55 test files / 266 tests, and browser smoke evidence is recorded in `docs/browser-screenshots/local-home-20260717-turn-replay.jpg`. This closes only the turn/message reservation boundary; full workflow transactionality, provider-side idempotency, durable attachment cleanup/round trips, and production deployment controls remain open.
