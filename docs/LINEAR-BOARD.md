...
The Postgres migration work is tracked in [ONE-254](https://linear.app/onecomputer/issue/ONE-254/platp1-implement-postgres-drizzle-repository-and-migrationimport-proof). The current branch contains the reviewed Drizzle schema, three generated SQL migrations, an explicit owner-required `db:import` seam, pre-transaction validation that rejects missing or cross-owner task/schedule project references, a disposable PostgreSQL 18 migration/import/restart proof, a CI PostgreSQL migration smoke job, and a fail-closed driver guard that rejects an unavailable Postgres selection or ambiguous `DATABASE_URL`. The running TaskStore remains SQLite-backed until transaction-compatible repositories, controlled runtime selection, and production idempotency/connection evidence are complete.
