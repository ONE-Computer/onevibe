...
ONE-254 progress note: commit `34a48ff` added standalone-message and atomic native-event persistence; commit `3a96539` added transaction-backed Postgres fork history with `forkHistoryAtomic=true`; the current workspace slice adds durable task bytes/version compare/restore/copy with restart recovery. Project-file revisions, attachment/preview/screenshot storage, and the controlled production driver switch remain open.
