...
### Changed
- V2 theme foundation (`src/theme/default.css`, `src/index.css`, `scripts/theme-literals.test.ts`): the obfuscated `--theme-color-*/--theme-effect-*` token layer (267 cryptic definitions) is deleted — every reference in the legacy CSS is inlined to its literal value (lossless: the layer was dark-only with no light overrides); `src/theme/default.css` is now the sole token source with the V2 semantic layer (`--surface-*`, `--text-*`, `--border-*`, `--accent`, `--status-*`, `--radius-*`, `--space-*`) plus `[data-theme="light"]` overrides. The token-boundary test is rewritten for the new architecture: the obfuscated system must stay deleted, canonical tokens stay in `default.css`, and every `src/v2/**/*.module.css` must be token-pure (no hex/rgba, no px font-size, no `!important`). Added `scripts/v2-qa.mjs` (Puppeteer harness: per-view × desktop/mobile × dark/light → `docs/qa-screenshots/`), `src/v2/CONVENTIONS.md` (V2 component/style rulebook), and puppeteer as a devDependency. Gate: 396 tests / 72 files ✓
...
- Short-view viewport fill (UI-03, `src/index.css`): `.appearance-view`, `.homepage-editor-view`, and `.capabilities-view` now have `min-height:calc(100vh - 64px)` (`60px` topbar ≤960px) with `display:flex;flex-direction:column`, so empty/short states fill the viewport instead of leaving a large dark void below the content (the report's 56px topbar figure was stale — the live topbar is 64px/60px)
- Unified view eyebrow labels (UI-04, `src/index.css`, `src/components/{SkillsLibrary,Artefacts,Capabilities,Appearance,HomepageEditor,Computers}.tsx`): new `.view-eyebrow` pattern (block, 500 10px, uppercase, `.10em` tracking, `--text-faint`, 6px bottom margin) now sits above the h1 on all six report-named views — their 12 eyebrow spans move from the conversation-header `task-kicker` class to `view-eyebrow`; `task-kicker` remains on the task conversation header and login card, which are out of the report's scope
- Unified view eyebrow labels (UI-04, `src/index.css` + eight view components): new `.view-eyebrow` class (`display:block`, 10px/500, uppercase, `.10em` tracking, `var(--text-faint)`, 6px bottom margin) replaces `task-kicker` on the header eyebrow spans of Skills, Artefacts, Capabilities, Appearance, Homepage, Computers, Library, and Schedules — the existing spans were visually invisible as eyebrows because global `!important` normalization rules stripped their uppercase/tracking; the conversation-header and login `task-kicker` usages are unchanged
